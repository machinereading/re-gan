...
class Embeddings(object):
    ...
    def __init__(self, name, path='../data/kaist/embedding/embedding-registry.json', lang='ko', extension='vec',
                 use_ELMo=False):
        self.name = name
        self.embed_size = 0
    ...
    def make_ELMo(self):
        ...
        if description is not None:
            ...
            # self.character_ids = tf.placeholder(tf.int32, shape=(None, 242, 50))
        ...
        for i in range(0, len(token_list)):
            for j in range(0, len(token_list[i])):
                ...
                concatenated_result[i][j] = np.concatenate(
                    (elmo_result[i][j], self.get_word_vector(token_list[i][j]).astype('float32')), )
    ...
    def get_ELMo_lmdb_vector(self, token_list, max_size_sentence):
        ...
        try:
                ...
                for i in range(0, len(token_list)):
                    ...
                    if vector:
                        ...
                        if local_embeddings.shape[0] > max_size_sentence - 2:
                            ...
                            filler = np.zeros((max_size_sentence - (local_embeddings.shape[0] + 2), ELMo_embed_size),
                                              dtype='float32')
            ...
            # when opening new transaction !
            self.env_ELMo.close()
            self.env_ELMo = lmdb.open(self.embedding_ELMo_cache, readonly=True, max_readers=2048, max_spare_txns=2,
                                      lock=False)
...
def test():
    embeddings = Embeddings("elmo-test", use_ELMo=True)

    token_list = [
        ['지미/Noun', '카터/Noun', '는/Josa', '조지아_주/Entity', '섬터/Noun', '카운티/Noun', '플레인스/Noun', '마을/Noun', '에서/Josa',
         '태어나다/Verb', './Punctuation']]

    vect = embeddings.get_sentence_vector_only_ELMo(token_list)
    print(vect)
