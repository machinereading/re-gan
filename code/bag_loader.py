...
import pickle
import time
from embedding.elmo_loader import Embeddings as ELMo
...
class loader:
    def __init__(self, relation_file, label_data_file, unlabel_data_file=None, group_eval_data_file=None,
        ...
        """
        np.random.seed(split_seed)  # create random seed
        self.n_vocab = n_vocab  # number of vocabulary in pre-trained word embedding
        self.n_test = valid_split  # number of validation data
        ...
        if unlabel_data_file is not None and use_DS_data:
            ...
            for fname in unlabel_data_file:
                with open(fname, 'rb') as f:
                    text, entity, pos, rel = pickle.load(f)
                self.unlabel_data += [(t, e, p, self.rel_ind[r]) for t, e, p, r in zip(text, entity, pos, rel) if
                                      len(t) < max_len]
            print('  -> done! elapsed = {}'.format(time.time() - ts))
        ...
        if embed_dir is None:
            # read ELMo vocab
            self.elmo = ELMo("elmo-test", use_ELMo=True)
            self.embed = None
    ...
    def new_epoch(self):
        np.random.shuffle(self.train_data)  # unlabeled data
        self.train_ptr = 0
        self.test_ptr = 0
        self.eval_ptr = 0
    ...
    def next_batch(self, data_source='train'):
        ...
        if data_source == 'train':  # move pointer
        ...
        if self.elmo is not None:
            local_token_ids = self.elmo.batcher.batch_sentences(text_list)
        else:
            local_token_ids = None

        return effective, X, Y, E, length, shapes, mask, local_token_ids
