...
class BAGRNN_Model:
    def __init__(self,
        ...
        self.enc_dim = enc_dim  # rnn cell size
        if rel_dim is None:  # PCNN = 3-fold, Bi-LSTM or Bi-GRU
    ...
    def build(self, is_training,
        ...
        if self.max_dist_embed is None:
            self.ent_embed = tf.constant(
                np.array([[0] * ent_dim * 2, [1] * ent_dim + [0] * ent_dim, [0] * ent_dim + [1] * ent_dim],
                         dtype=np.float32),
                dtype=tf.float32)
        else:
        ...
        if self.pretrain_word_embed is not None:
            ...
            # ELMo initializer
            self.character_ids = tf.placeholder(tf.int32, shape=(None, None, 50))
        ...
        if self.pretrain_word_embed is not None:
            ...
            self.orig_inputs = orig_inputs = mc.get_elmo_embedding(self.character_ids, self.elmo, self.sent_len)
        ...
        def discriminative_net(word_inputs, name='discriminative-net', reuse=False,
            ...
            with tf.variable_scope(name, reuse=reuse):
                ...
                if not use_pcnn:  # use RNN
                    ...
                    conv_out = tf.expand_dims(conv_out, axis=-1)  # [batch, feat, L, 1]
        ...
        self.probs, self.raw_loss = discriminative_net(orig_inputs, reuse=False,
                                                       only_pos_rel_loss=(self.adv_eps is not None) and (
                                                           self.only_perturb_pos_rel) and (not use_softmax_loss))
        if self.adv_eps is None:
            ...
            if self.adv_type == 'sent':
                ...
                self.perturb = perturb = self.adv_eps * tf.stop_gradient(
                    tf.nn.l2_normalize(raw_perturb * tf.expand_dims(mask, axis=-1), axis=[1, 2]))
            elif self.adv_type == 'batch':
                # normalize the whole batch
                self.perturb = perturb = self.adv_eps * tf.stop_gradient(
                    tf.nn.l2_normalize(raw_perturb * tf.expand_dims(mask, axis=-1), axis=[0, 1, 2]))
            ...
            self.perturb_probs, self.loss = discriminative_net(perturb_inputs,
                                                               reuse=True)  # optimize the loss with perturbed loss
