...
class BagTrainer:
    ...
    def feed_dict(self, source='train'):
        ...
        self.effective, X, Y, E, length, shapes, mask, local_token_ids = loader.next_batch(source)
        ...
        if M.elmo is None:
            ...
            fd[M.character_ids] = local_token_ids
    ...
    def evaluate(self, sess, stats_file='./stats/eval_stats.pkl', max_relations=None, incl_conf=False):
        ...
        # bestIdx = f1_score.index(best_f1)
        logger.print('  -> Done! Time Elapsed = {}s'.format(time.time() - ts))
        logger.print('>>>> Best F1 Score = %.5f' % best_f1)
        # logger.print('>>>> Best Precision = %.5f' % precis[bestIdx])
        # logger.print('>>>> Best Recall = %.5f' % recall[bestIdx])
    ...
    def train(self,
        ...
        with tf.Session(config=config) as sess:
            try:
                ...
                with open(model_dir + '/{m}_killed_iter{i}-counter.pkl'.format(m=name, i=global_batch_counter),
                          'wb') as f:
                    pickle.dump([global_batch_counter, global_ep_counter], f)
                logger.print('Model saved in file: %s' % save_path)
